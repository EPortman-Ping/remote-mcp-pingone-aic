 ...
 */
const validatePingAicToken = async(token: string): Promise<JWTPayload> => {
...
export const authenticationMiddleware = createMiddleware<{Bindings: Env, Variables: Props}>(async (c, next) => {
  try {
    ...
    const subjectClaims = await validatePingAicToken(subjectToken);
    c.executionCtx.props = { subjectClaims, subjectToken } as Props;
    await next();
  } catch {
    const metadataUrl = `${env.PING_AIC_ISSUER}/.well-known/oauth-protected-resource`;
 ...
 */
export const getActorToken = async (actorScopes: Array<string>): Promise<string> => {
