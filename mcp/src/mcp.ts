 ...
 * (delegation) to obtain a token properly audienced for any upstream APIs.
 */
export class TodoMcpServer extends McpAgent<Env, unknown, Props> {
  async init() {};

  private todoApiClient: TodoApiClient = new TodoApiClient();
  ...
  get server() {
    ...
    srv.registerTool(
      ...
      {
        description: 'Get the token claims the MCP server will use on the upstream API on behalf of the current session, helpful for debugging.',
      },
      async () => {
        if (!this.props) {
          const mcpResponse: McpToolContent = { type: 'text', text: 'Error: User session not found.' };
        ...
        try {
          ...
          const actorToken = await getActorToken(actorScopes);
          const apiToken = await exchangeForTodoApiToken(this.props.subjectToken, actorToken, actorScopes);
